...
const compact = (summary: LootSummary): CompactLootSummary => ({
  v: summary.version,
...
export function encodeLootSummary(summary: LootSummary): string {
  const compressed = zlibSync(strToU8(JSON.stringify(compact(summary))), { level: 9 });
...
export function decodeLootSummary(token: string): LootSummary | null {
  try {
    ...
    const value = JSON.parse(strFromU8(inflated)) as CompactLootSummary;
    if (value.v !== LOOT_SUMMARY_VERSION || !Array.isArray(value.r) || !Array.isArray(value.c) || !Array.isArray(value.t)) return null;
    if (value.r.length > LOOT_SUMMARY_ROW_LIMIT || value.c.length > ITEM_CATEGORIES.length || value.t.length !== 13) return null;
    if (!value.r.every((row) => Array.isArray(row) && row.length >= 5
    ...
    if (!value.c.every((entry) => Array.isArray(entry) && entry.length === 2
      && isLootCategory(entry[0]) && Number.isFinite(Number(entry[1])) && Number(entry[1]) >= 0)) return null;
    if (!value.t.every((entry) => Number.isFinite(Number(entry)))) return null;
    const [hasBaseline, csvPositive, csvNegative, csvNet, csvAdjustment,
      ...
      omittedCsvRows, omittedCsvValue, omittedManualRows, omittedManualValue] = value.t;
    const rows: LootSummaryRow[] = value.r.map((row) => ({
    ...
    const categories = value.c.map(([category, amount]) => ({ category, value: rounded(Number(amount)) }));
    return {
