...
- **Sessions** — integrity-checked local file auto-save with bounded Version history, contextual Undo, and Recently Deleted recovery. Compare 2-3 saved sessions side by side or use portable JSON export/import. The Sessions panel shows save status and repository size and can open the complete local data folder for backup. Panels are draggable and the layout persists there too.
